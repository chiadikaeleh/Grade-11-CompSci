/*
 * Name: Chiadika Eleh
 * Date: April 30th, 2019
 * Version: v1.00
 * Description: 
 * Using an age inputted, this program will determine whether it is old enough to 
 * enter the CCC. 
 */
package edu.hdsb.gwss.chiadika.ics3u.u5;

import java.io.File;
import java.io.FileNotFoundException;
import java.io.PrintWriter;
import java.util.Scanner;
import java.util.StringTokenizer;

/**
 *
 * @author chiadika
 */
public class AgeGate {

    /**
     * @param args the command line arguments
     */
    public static void main(String[] args) throws FileNotFoundException {

        // OBJECTS AND VARIABLES
        File file = new File("AgeGate.txt"); // Create File, and Scanner to File
        Scanner input = new Scanner(file);

        File outputFile = new File("OUT11.txt"); // Create File and Create a File Writer
        PrintWriter output = new PrintWriter(outputFile);

        String birthday, month, day, year;
        int dayvalue, monthvalue, yearvalue;
        
        // SPLASH PAGE
        System.out.println("AGE GATE");
        System.out.println("------------------------------------------------------");
        System.out.println("This program will deteremine if someone is old enough"
                + "to enter the CCC");
        
        // PROCESSING
        while (input.hasNext()) { // Read from File, until you reach the end of the file
            birthday = input.nextLine();
            StringTokenizer line = new StringTokenizer(birthday);

            while (line.hasMoreTokens()) {
                month = line.nextToken();
                monthvalue = Integer.parseInt(month);
                day = line.nextToken();
                dayvalue = Integer.parseInt(day);
                year = line.nextToken();
                yearvalue = Integer.parseInt(year);

                if (2019 - yearvalue >= 14) {
                    if (4 - monthvalue > 0) {
                        output.println("old enough");
                    } else if (4 - monthvalue == 0) {
                        if (25 - dayvalue >= 0) {
                            output.println("old enough");
                        } else {
                            output.println("too young");
                        }
                    }
                } else {
                    output.println("too young");
                }
            }
        }
        // Close File
        input.close();
        output.close();
    }
}
